 ...
 * @author Michel
 */
public class CPropellerServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        File blocklyAppFile = File.createTempFile("blocklyapp", ".c");
        try {
        ...
        if (action == PropellerPostAction.COMPILE) {
            ...
            result.setSucces(BlocklyProp.getCCompiler().compile(blocklyAppFile));
            result.setMessage(BlocklyProp.getCCompiler().getLastOutput());
            result.setCode(BlocklyProp.getCCompiler().getLastExitValue());
            ...
            boolean compileSuccess = BlocklyProp.getCCompiler().wasLastSuccess();
            compileResult.setSucces(compileSuccess);
            compileResult.setMessage(BlocklyProp.getCCompiler().getLastOutput());
            compileResult.setCode(BlocklyProp.getCCompiler().getLastExitValue());
    ...
    private boolean compileAndRun(PropellerPostAction action, File blocklyAppFile, String comPort) throws IOException {
        ...
        switch (action) {
            case LOAD_RAM:
                compiledFile = File.createTempFile("blocklyapp", ".elf");
                success = BlocklyProp.getCCompiler().compileForRam(blocklyAppFile, compiledFile);
                break;
            case LOAD_EEPROM:
                compiledFile = File.createTempFile("blocklyapp", ".elf");
                success = BlocklyProp.getCCompiler().compileForEeprom(blocklyAppFile, compiledFile);
                break;
        }
        if (success) {
            switch (action) {
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        switch (action) {
            case LOAD_RAM_BUF:
                blocklyAppFile = File.createTempFile("blocklyapp", ".elf");
                break;
