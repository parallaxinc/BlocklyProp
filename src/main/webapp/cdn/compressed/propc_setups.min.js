...
$(document).ready(function(){find_client();$("#client-danger-span").addClass("hidden");$("#client-warning-span").addClass("hidden");$("#client-unknown-span").addClass("hidden");$("#client-warning-download-links").html($("#client-download-links").html())});
...
$("#prop-btn-graph").addClass("disabled"),"searching"===a?($("#client-available").addClass("hidden"),$("#client-searching").removeClass("hidden"),$("#client-unavailable").addClass("hidden")):($("#client-available").addClass("hidden"),$("#client-searching").addClass("hidden"),$("#client-unavailable").removeClass("hidden")))},check_client=function(){$(".bpc-version").addClass("hidden");"ws"!==client_use_type&&$.get(client_url,function(a){client_available||(client_version=version_as_number(a.version),
a.server&&"BlocklyPropHTTP"===a.server?client_version<version_as_number(client_min_version)?($("#client-danger-span").removeClass("hidden"),$(".client-required-version").html(client_min_version),$(".client-your-version").html(a.version),$("#client-version-modal").modal("show")):client_version<version_as_number(client_recommended_version)&&($("#client-warning-span").removeClass("hidden"),$(".client-required-version").html(client_recommended_version),$(".client-your-version").html(a.version),$("#client-version-modal").modal("show")):
($("#client-unknown-span").removeClass("hidden"),$(".client-required-version").html(client_min_version),$(".client-your-version").html(a.version||"<b>UNKNOWN</b>"),$("#client-version-modal").modal("show")),client_use_type="http",client_available=!0,set_ui_buttons("available"),check_com_ports&&"function"===typeof check_com_ports&&(check_com_ports(),check_com_ports_interval=setInterval(check_com_ports,5E3)));setTimeout(check_client,2E4)}).fail(function(){clearInterval(check_com_ports_interval);client_use_type=
"none";ports_available=client_available=!1;set_ui_buttons("unavailable");check_ws_socket_timeout=setTimeout(find_client,3E3)})},connection_heartbeat=function(){if("ws"===client_use_type){var a=(new Date).getTime();client_ws_heartbeat+12E3<a&&(client_ws_connection.close(),lostWSConnection())}},configure_client=function(){var a=$("<form/>",{"class":"form-inline"});$("<span/>",{"class":"space_right"}).text("http://").appendTo(a);var b=$("<div/>",{"class":"form-group"}).appendTo(a);$("<input/>",{id:"domain_name",
