...
a("es6-impl",function(){return!0});a("es7",function(){return b("2 ** 2 == 4")});a("es8",function(){return b("async () => 1, true")});return c};var cursorX=0,cursorY=0,cursorGotoX=0,cursorGotoY=0,termSetCursor=0,textContainer=[""],term_been_scrolled=!1,fontSize="14px",termPxWide="560px",termPxHigh="380px",lineHeight=Math.floor(1.1*parseInt(fontSize.replace("px",""))),term_width=Math.floor(parseInt(fontSize.replace("px","")))/1.65,term_height=Math.floor(parseInt(termPxHigh.replace("px","")));term_width=Math.floor((parseInt(termPxWide.replace("px",""))-25)/term_width);
...
""))})});function processKey(a){null!==active_connection&&"simulated"!==active_connection&&"websocket"!==active_connection?(active_connection.send(String.fromCharCode(a)),trap_echos&&echo_trap.push(a)):"simulated"===active_connection?updateTermBox(String.fromCharCode(a)):"websocket"===active_connection&&(a={type:"serial-terminal",outTo:"terminal",portPath:getComPort(),baudrate:baudrate.toString(10),msg:String.fromCharCode(a),action:"msg"},client_ws_connection.send(JSON.stringify(a)))}
function displayInTerm(a){var b=terminal_buffer.length;terminal_buffer+=a;0===b?sendBufferToTerm():255<b&&!1===bufferAlert&&termBufferWarning()}function termBufferWarning(){bufferAlert=!0;var a=document.getElementById("serial-conn-info"),b=a.innerHTML;a.innerHTML='<span class="alert-danger">Your program is sending too much data to the terminal at once.<br>Try adding pauses or sending less data.</span>';setTimeout(function(){a.innerHTML=b;bufferAlert=!1},5E3)}
...
"scroll"),term_been_scrolled=!1);document.getElementById("serial_console").innerHTML=c.replace(/ /g,"&nbsp;").replace(/\r/g,"<br>")}function setCursor(a,b){a>term_width-1&&(a%=term_width);if(!textContainer[b])for(t=textContainer.length;t<=b;t++)textContainer[t]="";for(;!textContainer[b][a];)textContainer[b]+=" ";cursorX=a;cursorY=b;changeCursor(0,0)};var osName="unknown-client",inDemo=$("meta[name=in-demo]").attr("content");function nav(a,b,c){c=c||b;navigator[a]&&-1!==navigator[a].indexOf(b)&&(osName=c)}nav("appVersion","X11","UNIX");nav("appVersion","Mac","MacOS");nav("appVersion","Linux");nav("userAgent","Linux");nav("platform","Linux");nav("appVersion","Win","Windows");nav("userAgent","Windows");nav("platform","Win","Windows");nav("oscpu","Windows");nav("appVersion","CrOS","ChromeOS");$(document).ready(function(){$("body").addClass(osName)});var client_available=!1,client_url="http://localhost:6009/",client_version=0,client_domain_name="localhost",client_domain_port=6009,client_min_version=.2,client_baud_rate_min_version=.4,client_use_type="none",client_ws_connection=null,client_ws_heartbeat,client_ws_heartbeat_interval=null,baud_rate_compatible=!1,check_com_ports_interval=null,check_ws_socket_timeout=null,check_ws_client_timeout=null;$(document).ready(function(){find_client()});
var find_client=function(){check_ws_socket_timeout&&(clearTimeout(check_ws_socket_timeout),check_ws_client_timeout=null);establish_socket();"ws"!==client_use_type&&check_client()},version_as_number=function(a){a=a.toString().split(".");a.push("0");3>a.length&&(bootbox.alert("BlocklyProp is unable to determine what version of BlocklyPropClient is installed on your computer.\nYou may need to installor reinstall the BlocklyPropClient."),1===a.length?a="0.0.0":a.unshift("0"));return Number(a[0])<<20|
Number(a[1])<<10|Number(a[2])},set_ui_buttons=function(a){"available"===a?($("#client-available").removeClass("hidden"),$("#client-searching").addClass("hidden"),$("#client-unavailable").addClass("hidden"),$("#prop-btn-ram").removeClass("disabled"),$("#prop-btn-eeprom").removeClass("disabled"),$("#prop-btn-term").removeClass("disabled"),$("#prop-btn-graph").removeClass("disabled")):($("#prop-btn-ram").addClass("disabled"),$("#prop-btn-eeprom").addClass("disabled"),$("#prop-btn-term").addClass("disabled"),
$("#prop-btn-graph").addClass("disabled"),"searching"===a?($("#client-available").addClass("hidden"),$("#client-searching").removeClass("hidden"),$("#client-unavailable").addClass("hidden")):($("#client-available").addClass("hidden"),$("#client-searching").addClass("hidden"),$("#client-unavailable").removeClass("hidden")))},check_client=function(){"ws"!==client_use_type&&$.get(client_url,function(a){client_available||(console.log(a),client_version=version_as_number(a.version),a.server&&"BlocklyPropHTTP"===
a.server&&client_version<version_as_number(client_min_version)&&bootbox.alert("This now requires at least version "+client_min_version+" of BlocklyPropClient."),version_as_number(a.version)>=version_as_number(client_baud_rate_min_version)&&(baud_rate_compatible=!0),client_use_type="http",client_available=!0,set_ui_buttons("available"),check_com_ports&&"function"===typeof check_com_ports&&(check_com_ports(),check_com_ports_interval=setInterval(check_com_ports,5E3)));setTimeout(check_client,2E4)}).fail(function(){clearInterval(check_com_ports_interval);
client_use_type="none";client_available=!1;set_ui_buttons("unavailable");check_ws_socket_timeout=setTimeout(find_client,3E3)})},connection_heartbeat=function(){if("ws"===client_use_type){var a=(new Date).getTime();client_ws_heartbeat+12E3<a&&(client_ws_connection.close(),lostWSConnection())}},configure_client=function(){var a=$("<form/>",{"class":"form-inline"});$("<span/>",{"class":"space_right"}).text("http://").appendTo(a);var b=$("<div/>",{"class":"form-group"}).appendTo(a);$("<input/>",{id:"domain_name",
type:"text","class":"form-control",value:client_domain_name}).appendTo(b);$("<span/>",{"class":"space_left space_right"}).text(":").appendTo(a);b=$("<div/>",{"class":"form-group"}).appendTo(a);$("<input/>",{id:"port_number",type:"number","class":"form-control",value:client_domain_port}).appendTo(b);bootbox.dialog({title:"Configure BlocklyPropClient",message:a,buttons:{cancel:{label:"Cancel",className:"btn-default"},save:{label:"Save",className:"btn-success",callback:function(){client_domain_name=
$("#domain_name").val();client_domain_port=$("#port_number").val();client_url="http://"+client_domain_name+":"+client_domain_port+"/"}}}})};
function establish_socket(){if(!client_available){var a=$("#comPort").val();$("#comPort").empty();$("#comPort").append($("<option>",{text:"Searching..."}));select_com_port(a);var b=client_url.replace("http","ws"),c=new WebSocket(b);c.onopen=function(){null!==client_ws_connection&&client_ws_connection.close();var a={type:"hello-browser",baud:baudrate};client_ws_connection=c;c.send(JSON.stringify(a))};c.onerror=function(a){console.log("WebSocket Error");console.log(a)};c.onmessage=function(b){b=JSON.parse(b.data);
if("hello-client"===b.type)client_version=version_as_number(b.version),console.log("Websocket client found - version "+b.version),version_as_number(b.version)<version_as_number(client_min_version)&&bootbox.alert("This now requires at least version "+client_min_version+" of BlocklyPropClient."),version_as_number(b.version)>=version_as_number(client_baud_rate_min_version)&&(baud_rate_compatible=!0),client_use_type="ws",client_available=!0,set_ui_buttons("available"),b=JSON.stringify({type:"port-list-request",
msg:"port-list-request"}),c.send(b);else if("port-list"===b.type){client_ws_heartbeat=(new Date).getTime();client_ws_heartbeat_interval||(client_ws_heartbeat_interval=setInterval(connection_heartbeat,4E3));a=$("#comPort").val();$("#comPort").empty();if(0<b.ports.length)for(var d=0;d<b.ports.length;d++)$("#comPort").append($("<option>",{text:b.ports[d]}));else $("#comPort").append($("<option>",{text:"No devices found"}));select_com_port(a)}else"serial-terminal"===b.type&&("string"===typeof b.msg||
b.msg instanceof String)?(d=atob(b.msg),void 0!==b.msg&&(null!==term?(displayInTerm(d),$("#serial_console").focus()):null!==graph&&graph_new_data(d))):"ui-command"===b.type?"open-terminal"===b.action?serial_console():"open-graph"===b.action?graphing_console():"close-terminal"===b.action?($("#console-dialog").modal("hide"),newTerminal=!1,updateTermBox(0)):"close-graph"===b.action?($("#graphing-dialog").modal("hide"),graph_reset()):"clear-compile"===b.action?$("#compile-console").val(""):"message-compile"===
b.action?$("#compile-console").val($("#compile-console").val()+b.msg):"close-compile"===b.action?($("#compile-dialog").modal("hide"),$("#compile-console").val("")):"console-log"===b.action?console.log(b.msg):"websocket-close"===b.action?client_ws_connection.close():"alert"===b.action&&alert(b.msg):console.log("Unknown WS msg: "+JSON.stringify(b))};c.onClose=function(){lostWSConnection()}}}
function lostWSConnection(){client_ws_connection=null;client_use_type="none";client_available=!1;set_ui_buttons("unavailable");term=null;newTerminal=!1;selected_port=$("#comPort").val();$("#comPort").empty();$("#comPort").append($("<option>",{text:"Searching..."}));select_com_port(selected_port);client_ws_heartbeat_interval&&(clearInterval(client_ws_heartbeat_interval),client_ws_heartbeat_interval=null);check_ws_socket_timeout=setTimeout(find_client,3E3)};var BlocklyProp={},TABS_=["blocks","propc","xml"],selected="blocks",term=null,graph=null,codePropC=null,codeXml=null,baudrate=115200,graph_temp_data=[],graph_data_ready=!1,graph_connection_string="",graph_timestamp_start=null,graph_timestamp_restart=0,graph_paused=!1,graph_temp_string=new String,graph_time_multiplier=0,graph_interval_id=null,fullCycleTime=53.6870912,graph_labels=null,graph_csv_data=[],console_header_arrived=!1,console_header=null,active_connection=null,connString="",connStrYet=!1,
graph_options={showPoint:!1,fullWidth:!0,axisX:{type:Chartist.AutoScaleAxis,onlyInteger:!0},refreshRate:250,sampleTotal:40},graph_data={series:[[],[],[],[],[],[],[],[],[],[]]};
function tabClick(a){for(var b in TABS_)document.getElementById("content_"+TABS_[b]).style.display="none";selected=a.replace("tab_","");b=document.getElementsByClassName("blocklyToolboxDiv");var c=document.getElementsByClassName("btn-view-code");document.getElementById("btn-view-blocks").style.display="none";document.getElementById("menu-save-as-propc")&&(document.getElementById("menu-save-as-propc").style.display="none");for(var d=0;d<c.length;d++)c[d].style.display="none";if("propcfile"!==projectData.board)if("tab_blocks"===
a){for(d=0;d<c.length;d++)c[d].style.display="inline";for(a=0;a<b.length;a++)b[a].style.display="block"}else{for(a=0;a<b.length;a++)b[a].style.display="none";document.getElementById("btn-view-blocks").style.display="inline";document.getElementById("menu-save-as-propc")&&(document.getElementById("menu-save-as-propc").style.display="block")}else"true"===$("#editor-full-mode")&&(document.getElementById("prop-btn-graph").style.display="none",document.getElementById("upload-project").style.display="none"),
document.getElementById("prop-btn-pretty").style.display="inline-block",document.getElementById("prop-btn-find-replace").style.display="inline-block",document.getElementById("prop-btn-undo").style.display="inline-block",document.getElementById("prop-btn-redo").style.display="inline-block",document.getElementById("download-project").style.display="none";"propcfile"!==projectData.board?document.getElementById("content_"+selected).style.display="block":document.getElementById("content_propc").style.display=
"block";renderContent()}function renderContent(){var a=document.getElementById("content_"+selected);"content_blocks"===a.id?Blockly.mainWorkspace.render():"content_xml"===a.id?(a=Blockly.Xml.workspaceToDom(Blockly.mainWorkspace),a=Blockly.Xml.domToPrettyText(a),codeXml.setValue(a),codeXml.gotoLine(0)):"content_propc"===a.id&&prettyCode(Blockly.propc.workspaceToCode(Blockly.mainWorkspace))}
var prettyCode=function(a){a||(a=codePropC.getValue());a=js_beautify(a,{brace_style:"expand",indent_size:2});a=a.replace(/,\n[\s\xA0]+/g,", ").replace(/, & /g,", &").replace(/, \* /g,", *").replace(/\( & /g,"(&").replace(/\( \* /g,"(*").replace(/char \* /g,"char *").replace(/fdserial \* /g,"fdserial *").replace(/colorPal \* /g,"colorPal *").replace(/ws2812 \* /g,"ws2812 *").replace(/\)\s*[\n\r]\s*{/g,") {").replace(/\[([0-9]*)\]\s*=\s*{\s*([0-9xXbBA-F,\s]*)\s*};/g,"[$1] = {$2};");codePropC.setValue(a);
...
function init(a){codePropC=ace.edit("code-propc");codePropC.setTheme("ace/theme/chrome");codePropC.getSession().setMode("ace/mode/c_cpp");codePropC.getSession().setTabSize(2);codePropC.setReadOnly(!0);codeXml=ace.edit("code-xml");codeXml.setTheme("ace/theme/chrome");codeXml.getSession().setMode("ace/mode/xml");codeXml.setReadOnly(!0);window.Blockly=a;loadProject();"propcfile"===projectData.board&&(codePropC.setReadOnly(!1),codePropC.commands.addCommand({name:"undo",bindKey:{win:"Ctrl-z",mac:"Command-z"},
exec:function(a){a.undo()},readOnly:!0}),codePropC.commands.addCommand({name:"redo",bindKey:{win:"Ctrl-y",mac:"Command-y"},exec:function(a){a.redo()},readOnly:!0}),codePropC.commands.addCommand({name:"find_replace",bindKey:{win:"Ctrl-f",mac:"Command-f"},exec:function(){findReplaceCode()},readOnly:!0}),tabClick("tab_propc"))}function setBaudrate(a){baudrate=a}
function cloudCompile(a,b,c){var d="";d=codePropC.getReadOnly()?Blockly.propc.workspaceToCode(Blockly.mainWorkspace):codePropC.getValue();if(-1<d.indexOf("EMPTY_PROJECT"))alert("You can't compile an empty project");else{$("#compile-dialog-title").text(a);$("#compile-console").val("");$("#compile-dialog").modal("show");d=js_beautify(d,{brace_style:"expand",indent_size:2});var e=!1;-1<d.indexOf("SERIAL_TERMINAL USED")?e="term":-1<d.indexOf("SERIAL_GRAPHING USED")&&(e="graph");$.ajax({method:"POST",
...
"";"compile"!==b&&(d="\nLoading program on the Propeller - Please Wait...\n");a.success?($("#compile-console").val(a["compiler-output"]+a["compiler-error"]+d),c(a,e)):$("#compile-console").val(a["compiler-output"]+a["compiler-error"]+d)}}).fail(function(a){console.log(a);"string"===typeof a?alert("BlocklyProp was unable to compile your project:\n----------\n"+a+'\nIt may help to "Force Refresh" by pressing Control-Shift-R (Windows/Linux) or Shift-Command-R (Mac)'):alert("BlocklyProp was unable to compile your project:\n----------\n"+
...
function loadInto(a,b,c){client_available?cloudCompile(a,b,function(a,b){if("ws"===client_use_type){var d="none";if("term"===b||"graph"===b)d=b;d={type:"load-prop",action:c,payload:a.binary,debug:d,extension:a.extension,portPath:getComPort()};client_ws_connection.send(JSON.stringify(d))}else $.post(client_url+"load.action",{action:c,binary:a.binary,extension:a.extension,comport:getComPort()},function(a){$("#compile-console").val($("#compile-console").val()+a.message);console.log(a);"term"===b&&a.success?
serial_console():"graph"===b&&a.success&&graphing_console()})}):alert('BlocklyPropClient not available to communicate with a microcontroller\nIt may help to "Force Refresh" by pressing Control-Shift-R (Windows/Linux) or Shift-Command-R (Mac)')}
function serial_console(){var a=!1;if("ws"!==client_use_type)if(null===term&&(term={portPath:getComPort()},a=!0),client_available){var b=client_url+"serial.connect";b=b.replace("http","ws");var c=new WebSocket(b);c.onopen=function(){connString="";connStrYet=!1;baud_rate_compatible&&baudrate?c.send("+++ open port "+getComPort()+" "+baudrate):c.send("+++ open port "+getComPort());active_connection=c};c.onerror=function(a){console.log("WebSocket Error");console.log(a)};c.onmessage=function(a){version_as_number("0.7.0")>
client_version||atob(a.data);connStrYet?displayInTerm(a.data):(connString+=a.data,-1<connString.indexOf(baudrate.toString(10))?(connStrYet=!0,document.getElementById("serial-conn-info")&&(document.getElementById("serial-conn-info").innerHTML=connString.trim())):displayInTerm(a.data));$("#serial_console").focus()};a||updateTermBox(0);c.onClose=function(){active_connection=null;connString="";connStrYet=!1};$("#console-dialog").on("hidden.bs.modal",function(){c.close();document.getElementById("serial-conn-info")&&
(document.getElementById("serial-conn-info").innerHTML="");updateTermBox(0);term_been_scrolled=!1;active_connection=term=null})}else active_connection="simulated",a&&(displayInTerm("Simulated terminal because you are in demo mode\n"),displayInTerm("Connection established with: "+getComPort()+"\n")),$("#console-dialog").on("hidden.bs.modal",function(){term_been_scrolled=!1;active_connection=null;updateTermBox(0);term=null});else if("ws"===client_use_type){term={portPath:getComPort()};a=!0;var d={type:"serial-terminal",
outTo:"terminal",portPath:getComPort(),baudrate:baudrate.toString(10),msg:"none",action:"msg"};!0===a?(d.action="open",active_connection="websocket",document.getElementById("serial-conn-info")&&(document.getElementById("serial-conn-info").innerHTML="Connection established with "+d.portPath+" at baudrate "+d.baudrate),client_ws_connection.send(JSON.stringify(d))):updateTermBox(0);$("#console-dialog").on("hidden.bs.modal",function(){"close"!==d.action&&(d.action="close",document.getElementById("serial-conn-info")&&
(document.getElementById("serial-conn-info").innerHTML=""),active_connection=null,client_ws_connection.send(JSON.stringify(d)));a=term_been_scrolled=!1;updateTermBox(0)})}$("#console-dialog").modal("show")}
...
low:Number(c[3]),high:Number(c[4])};"S"===c[2]&&(graph_options.sampleTotal=Number(c[1]));null===graph?(graph_reset(),graph_temp_string="",graph=new Chartist.Line("#serial_graphing",graph_data,graph_options),a=!0):graph.update(graph_data,graph_options);if("ws"!==client_use_type&&client_available){b=client_url+"serial.connect";b=b.replace("http","ws");var f=new WebSocket(b);f.onopen=function(){baud_rate_compatible&&baudrate?f.send("+++ open port "+getComPort()+" "+baudrate):f.send("+++ open port "+
getComPort())};f.onerror=function(a){console.log("WebSocket Error");console.log(a)};f.onmessage=function(a){graph_new_data(a.data)};if(a||null!==graph)graph_new_labels(),graph_interval_id=setInterval(function(){graph.update(graph_data);graph_update_labels()},graph_options.refreshRate);f.onClose=function(){graph_reset()};$("#graphing-dialog").on("hidden.bs.modal",function(){f.close();graph_reset()})}else if("ws"===client_use_type&&client_available){var g={type:"serial-terminal",outTo:"graph",portPath:getComPort(),
baudrate:baudrate.toString(10),msg:"none",action:"msg"};if(a||null!==graph)graph_new_labels(),graph_interval_id=setInterval(function(){graph.update(graph_data);graph_update_labels()},graph_options.refreshRate),g.action="open",client_ws_connection.send(JSON.stringify(g));$("#graphing-dialog").on("hidden.bs.modal",function(){"close"!==g.action&&(g.action="close",client_ws_connection.send(JSON.stringify(g)));graph_reset()})}$("#graphing-dialog").modal("show");document.getElementById("btn-graph-play")&&
...
var check_com_ports=function(){if("ws"!==client_use_type&&void 0!==client_url){var a=$("#comPort").val();$.get(client_url+"ports.json",function(b){$("#comPort").empty();for(var c=0;c<b.length;c++)$("#comPort").append($("<option>",{text:b[c]}));select_com_port(a);client_available=!0}).fail(function(){$("#comPort").empty();$("#comPort").append($("<option>",{text:"Searching..."}));select_com_port(a);client_available=!1})}},select_com_port=function(a){null!==a&&$("#comPort").val(a);null===$("#comPort").val()&&
0<$("#comPort option").size()&&$("#comPort").val($("#comPort option:first").text())};$(document).ready(function(){check_com_ports()});var getComPort=function(){return $("#comPort").find(":selected").text()};
...
String.prototype.endsWith||(String.prototype.endsWith=function(a,b){var c=this.toString();if("number"!==typeof b||!isFinite(b)||Math.floor(b)!==b||b>c.length)b=c.length;b-=a.length;c=c.indexOf(a,b);return-1!==c&&c===b});String.prototype.startsWith||(String.prototype.startsWith=function(a,b){return this.substr(b||0,a.length)===a});var baseUrl=$("meta[name=base]").attr("content"),cdnUrl=$("meta[name=cdn]").attr("content"),user_authenticated="true"===$("meta[name=user-auth]").attr("content")?!0:!1,projectData=null,ready=!1,projectLoaded=!1,uploadStaged=!1,ignoreSaveCheck=!1,last_saved_timestamp=0,last_saved_time=0,idProject=0,uploadedXML="";function getURLParameter(a){return decodeURIComponent(((new RegExp("[?|&]"+a+"=([^&;]+?)(&|#|;|$)")).exec(window.location.search)||[null,""])[1].replace(/\+/g,"%20"))||null}
$(document).ready(function(){user_authenticated?($(".auth-true").css("display",$(this).attr("displayas")),$(".auth-false").css("display","none")):($(".auth-false").css("display",$(this).attr("displayas")),$(".auth-true").css("display","none"));$(".url-prefix").attr("href",function(a,c){return baseUrl+c});var a=document.getElementsByTagName("img");for(l=0;l<a.length;l++)a[l].src=cdnUrl+a[l].getAttribute("data-src");(idProject=getURLParameter("project"))?$.get(baseUrl+"rest/shared/project/editor/"+
idProject,function(a){console.log(a);projectData=a;showInfo(a);projectLoaded=!0;ready&&(setProfile(a.board),initToolbox(a.board,[]));"true"===$("#editor-full-mode")&&("s3"===projectData.board?($("#prop-btn-ram").addClass("hidden"),document.getElementById("client-available").innerHTML=document.getElementById("client-available-short").innerHTML):($("#prop-btn-ram").removeClass("hidden"),document.getElementById("client-available").innerHTML=document.getElementById("client-available-long").innerHTML));
timestampSaveTime(20,!0);setInterval(checkLastSavedTime,6E4)}).fail(function(){utils.showMessage("Unable to Access Project","The BlocklyProp Editor was unable to access the project you requested.  If you are sure the project exists, you may need to contact the project's owner and ask them to share their project before you will be able to view it.",function(){window.location=baseUrl})}):window.location=baseUrl;$("#save-project").on("click",function(){saveProject()});$("#save-project-as").on("click",
function(){saveAsDialog()});$("#download-project").on("click",function(){downloadCode()});$("#upload-project").on("click",function(){uploadCode()});$("#save-check-dialog").on("hidden.bs.modal",function(){timestampSaveTime(5,!1)})});
var timestampSaveTime=function(a,b){var c=new Date;c.getTime()+6E4*a>last_saved_timestamp&&(last_saved_timestamp=c.getTime()+6E4*a,b&&(last_saved_time=c.getTime()))},checkLastSavedTime=function(){var a=new Date,b=a.getTime();a=Math.round((a.getTime()-last_saved_time)/6E4);$("#save-check-warning-time").html(a.toString(10));b>last_saved_timestamp&&checkLeave()&&"true"===$("#editor-full-mode").html()&&$("#save-check-dialog").modal("show")},showInfo=function(a){$(".project-name").text(a.name);a.yours||
$(".project-owner").text("("+a.user+")");var b=window.location;b=b.protocol+"//"+b.host+"/"+b.pathname.split("/")[1];$("#project-icon").html('<img src="'+b+{"activity-board":"/cdn/images/board-icons/IconActivityBoard.png",s3:"/cdn/images/board-icons/IconS3.png",heb:"/cdn/images/board-icons/IconBadge.png",flip:"/cdn/images/board-icons/IconFlip.png",other:"/cdn/images/board-icons/IconOtherBoards.png",propcfile:"/cdn/images/board-icons/IconC.png"}[a.board]+'"/>')},propcAsBlocksXml=function(){return'<xml xmlns="http://www.w3.org/1999/xhtml"><block type="propc_file" id="abcdefghijklmno12345" x="100" y="100"><field name="FILENAME">single.c</field><field name="CODE">'+
encodeXml(codePropC.getValue().replace("/* EMPTY_PROJECT */\n",""))+"</field></block></xml>"},saveProject=function(){var a="";"propcfile"===projectData.board?(a=propcAsBlocksXml(),clearXml(),loadToolbox(a)):a=getXml();projectData.code=a;$.post(baseUrl+"rest/project/code",projectData,function(b){var c=projectData.yours;projectData=b;projectData.code=a;c||(window.location.href=baseUrl+"projecteditor?id="+b.id)}).done(function(){var a=document.getElementById("save-project");a.style.paddingLeft="10px";
a.style.background="rgb(92, 184, 92)";a.style.borderColor="rgb(76, 174, 76)";setTimeout(function(){a.innerHTML="Save &#x2713;"},600);setTimeout(function(){a.innerHTML="Save&nbsp;&nbsp;";a.style.paddingLeft="15px";a.style.background="#337ab7";a.style.borderColor="#2e6da4"},1750)}).fail(function(){var a=document.getElementById("save-project");a.style.paddingLeft="10px";a.style.background="rgb(214, 44, 44)";a.style.borderColor="rgb(191, 38, 38)";setTimeout(function(){a.innerHTML="Save &times;"},600);
setTimeout(function(){a.innerHTML="Save&nbsp;&nbsp;";a.style.paddingLeft="15px";a.style.background="#337ab7";a.style.borderColor="#2e6da4"},1750);utils.showMessage("Not logged in","BlocklyProp was unable to save your project.\n\nYou may still be able to download it as a Blockls file.\n\nYou will need to return to the homepage to log back in.")});timestampSaveTime(20,!0)},saveAsDialog=function(){checkLeave()&&projectData.yours&&utils.confirm(Blockly.Msg.DIALOG_SAVE_TITLE,Blockly.Msg.DIALOG_SAVE_FIRST,
function(a){a&&saveProject()},"Yes","No");$("#save-as-project-name").val(projectData.name);$("#save-as-board-type").empty();profile["default"].saves_to.forEach(function(a){$("#save-as-board-type").append($("<option />").val(a[1]).text(a[0]))});"demo"===inDemo&&$("#save-as-board-type").append($("<option />").val("propcfile").text("Propeller C (code-only)"));$("#save-as-type-dialog").modal("show")},checkBoardType=function(){var a=projectData.board,b=$("#save-as-board-type").val();a===b||"propcfile"===
b?document.getElementById("save-as-verify-boardtype").style.display="none":document.getElementById("save-as-verify-boardtype").style.display="block"},saveProjectAs=function(){var a=$("#save-as-board-type").val(),b=$("#save-as-project-name").val(),c="";"propcfile"===projectData.board?(c=propcAsBlocksXml(),clearXml(),loadToolbox(c)):c=getXml();a===projectData.board?(projectData.code=c,projectData.name=b,$.post(baseUrl+"rest/project/code-as",projectData,function(a){projectData=a;projectData.code=c;window.location.href=
baseUrl+"projecteditor?id="+a.id})):(ignoreSaveCheck=!0,projectData.code=c,projectData.name=b,projectData.board="propcfile",$.post(baseUrl+"createproject",{"project-name":b,"board-type":a,"project-description":projectData.description,"project-type":projectData.type,sharing:projectData["private"]?"private":"shared","project-description-html":projectData["description-html"]},function(a){console.log(a);a.success?(projectData=a,window.location.href=baseUrl+"projecteditor?id="+a.id,projectData.code=c,
setTimeout(function(){$.post(baseUrl+"rest/project/code",projectData,function(a){projectData=a;projectData.code=c;ignoreSaveCheck=!1})},1E3)):"string"===typeof a.message?alert("There was an error when BlocklyProp tried to create your project:\n"+a.message):alert("There was an error when BlocklyProp tried to create your project:\n"+a.message.toString())}));timestampSaveTime(20,!0)},editProjectDetails=function(){window.location.href=baseUrl+"my/projects.jsp#"+idProject},blocklyReady=function(){getURLParameter("debug")&&
(document.getElementById("tab_xml").style.display="inline-block");projectLoaded?(setProfile(projectData.board),initToolbox(projectData.board,[])):ready=!0},loadProject=function(){null!==projectData&&!1===uploadStaged&&(43>projectData.code.length&&(projectData.code='<xml xmlns="http://www.w3.org/1999/xhtml"></xml>'),loadToolbox(projectData.code));"true"===$("#editor-full-mode")&&("s3"===projectData.board?($("#load-ram-button").addClass("hidden"),$("#open-graph-output").addClass("hidden"),document.getElementById("client-available").innerHTML=
document.getElementById("client-available-short").innerHTML):($("#load-ram-button").removeClass("hidden"),$("#open-graph-output").removeClass("hidden"),document.getElementById("client-available").innerHTML=document.getElementById("client-available-long").innerHTML))};window.onbeforeunload=function(){if(checkLeave())return Blockly.Msg.DIALOG_CHANGED_SINCE};
var checkLeave=function(){if(ignoreSaveCheck)return!1;var a=getXml();"propcfile"===projectData.board&&(a=propcAsBlocksXml());return null===projectData?'<xml xmlns="http://www.w3.org/1999/xhtml"></xml>'===a?!1:!0:projectData.code===a?!1:!0};setInterval(function(){$.get(baseUrl+"ping")},6E4);function hashCode(a){for(var b=0,c=0,d=a.length;c<d;)b=(b<<5)-b+a.charCodeAt(c++)<<0;return b+2147483647+1}
...
"block",document.getElementById("selectfile-replace").disabled=!0,document.getElementById("selectfile-append").disabled=!0,uploadedXML="")};b.readAsText(a[0]);""!==uploadedXML&&(uploadedXML='<xml xmlns="http://www.w3.org/1999/xhtml">'+uploadedXML+"</xml>",uploadStaged=!0)}
...
function replaceCode(){$("#upload-dialog").modal("hide");if(""!==uploadedXML){var a=uploadedXML;a=a.substring(42,a.length);a=a.substring(0,a.length-29);location.reload();setProfile(projectData.board);initToolbox(projectData.board,[]);projectData.code='<xml xmlns="http://www.w3.org/1999/xhtml">'+a+"</xml>";$(window).load(function(){loadToolbox(projectData.code);uploadStaged=!1});clearUploadInfo()}}
function appendCode(){$("#upload-dialog").modal("hide");if(""!==uploadedXML){var a=projectData.code;a=a.substring(42,a.length);a=a.substring(0,a.length-6);var b=uploadedXML;b=b.substring(42,b.length);b=b.substring(0,b.length-6);location.reload();setProfile(projectData.board);initToolbox(projectData.board,[]);projectData.code='<xml xmlns="http://www.w3.org/1999/xhtml">'+a+b+"</xml>";$(window).load(function(){loadToolbox(projectData.code);uploadStaged=!1});clearUploadInfo()}}
...
function initToolbox(a,b){filterToolbox(a,b);Blockly.inject("content_blocks",{toolbox:document.getElementById("toolbox"),trashcan:!0,media:cdnUrl+"blockly/media/",path:cdnUrl+"blockly/",comments:!1});init(Blockly)}function clearXml(){Blockly.mainWorkspace.clear()}function loadToolbox(a){a=Blockly.Xml.textToDom(a);Blockly.Xml.domToWorkspace(a,Blockly.mainWorkspace)}function getXml(){var a=Blockly.Xml.workspaceToDom(Blockly.mainWorkspace);return Blockly.Xml.domToText(a)}
function showOS(a){$("body").removeClass("Windows").removeClass("MacOS").removeClass("Linux").removeClass("ChromeOS");$("body").addClass(a)};