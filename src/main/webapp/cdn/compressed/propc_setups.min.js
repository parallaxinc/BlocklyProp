...
c=goog.global.$jscomp;d=c.transpile}}d||(d=c.transpile=function(a,c){goog.logToConsole_(c+" requires transpilation but no transpiler was found.");return a});return d(a,b)};
...
"scroll"),term_been_scrolled=!1);document.getElementById("serial_console").innerHTML=c.replace(/ /g,"&nbsp;").replace(/\r/g,"<br>")}function setCursor(a,b){a>term_width-1&&(a%=term_width);if(!textContainer[b])for(t=textContainer.length;t<=b;t++)textContainer[t]="";for(;!textContainer[b][a];)textContainer[b]+=" ";cursorX=a;cursorY=b;changeCursor(0,0)};var osName="unknown-client",inDemo=$("meta[name=in-demo]").attr("content");function nav(a,b,c){c=c||b;navigator[a]&&-1!==navigator[a].indexOf(b)&&(osName=c)}nav("appVersion","X11","UNIX");nav("appVersion","Mac","MacOS");nav("appVersion","Linux");nav("userAgent","Linux");nav("platform","Linux");nav("appVersion","Win","Windows");nav("userAgent","Windows");nav("platform","Win","Windows");nav("oscpu","Windows");nav("appVersion","CrOS","ChromeOS");$(document).ready(function(){$("body").addClass(osName)});var client_available=!1,ports_available=!1,client_url="http://localhost:6009/",client_version=0,version_control_info={},show_client_warning=!0,client_domain_name="localhost",client_domain_port=6009,client_min_version="0.6.0",client_use_type="none",client_ws_connection=null,client_ws_heartbeat,client_ws_heartbeat_interval=null,check_com_ports_interval=null,check_ws_socket_timeout=null,check_ws_client_timeout=null;$(document).ready(function(){find_client()});
...
$("#prop-btn-graph").addClass("disabled"),"searching"===a?($("#client-available").addClass("hidden"),$("#client-searching").removeClass("hidden"),$("#client-unavailable").addClass("hidden")):($("#client-available").addClass("hidden"),$("#client-searching").addClass("hidden"),$("#client-unavailable").removeClass("hidden")))},closeClientWarning=function(){show_client_warning=!1;$("#client-warning").css("display","none");Blockly.mainWorkspace.render()},check_client=function(){"ws"!==client_use_type&&
$.get(client_url,function(a){client_available||(client_version=version_as_number(a.version),a.server&&"BlocklyPropHTTP"===a.server&&client_version<version_as_number(client_min_version)&&bootbox.alert("This system now requires at least version "+client_min_version+" of BlocklyPropClient- yours is: "+a.version),client_use_type="http",client_available=!0,set_ui_buttons("available"),check_com_ports&&"function"===typeof check_com_ports&&(check_com_ports(),check_com_ports_interval=setInterval(check_com_ports,
5E3)));setTimeout(check_client,2E4)}).fail(function(){clearInterval(check_com_ports_interval);client_use_type="none";ports_available=client_available=!1;set_ui_buttons("unavailable");check_ws_socket_timeout=setTimeout(find_client,3E3)})},connection_heartbeat=function(){if("ws"===client_use_type){var a=(new Date).getTime();client_ws_heartbeat+12E3<a&&(client_ws_connection.close(),lostWSConnection())}},configure_client=function(){var a=$("<form/>",{"class":"form-inline"});$("<span/>",{"class":"space_right"}).text("http://").appendTo(a);
...
function lostWSConnection(){client_ws_connection=null;client_use_type="none";ports_available=client_available=!1;set_ui_buttons("unavailable");term=null;newTerminal=!1;set_port_list();client_ws_heartbeat_interval&&(clearInterval(client_ws_heartbeat_interval),client_ws_heartbeat_interval=null);check_ws_socket_timeout=setTimeout(find_client,3E3)};var BlocklyProp={},TABS_=["blocks","propc","xml"],selected="blocks",term=null,graph=null,codePropC=null,codeXml=null,baudrate=115200,graph_temp_data=[],graph_data_ready=!1,graph_connection_string="",graph_timestamp_start=null,graph_timestamp_restart=0,graph_paused=!1,graph_temp_string=new String,graph_time_multiplier=0,graph_interval_id=null,fullCycleTime=53.6870912,graph_labels=null,graph_csv_data=[],console_header_arrived=!1,console_header=null,active_connection=null,connString="",connStrYet=!1,
graph_options={showPoint:!1,fullWidth:!0,axisX:{type:Chartist.AutoScaleAxis,onlyInteger:!0},refreshRate:250,sampleTotal:40},graph_data={series:[[],[],[],[],[],[],[],[],[],[]]},minEnc64Ver=version_as_number("0.7.0"),minVer=version_as_number(client_min_version);
...
"inline-block",document.getElementById("download-project").style.display="none";document.getElementById("content_"+selected).style.display="block";"propcfile"===projectData.board&&"xml"===selected&&getURLParameter("debug")?(document.getElementById("btn-view-propc").style.display="inline",document.getElementById("btn-view-xml").style.display="none"):"propcfile"===projectData.board&&"propc"===selected&&getURLParameter("debug")&&(document.getElementById("btn-view-xml").style.display="inline",document.getElementById("btn-view-propc").style.display=
...
a.user+")");$("#project-icon").html('<img src="'+cdnUrl+{"activity-board":"images/board-icons/IconActivityBoard.png",s3:"images/board-icons/IconS3.png",heb:"images/board-icons/IconBadge.png",flip:"images/board-icons/IconFlip.png",other:"images/board-icons/IconOtherBoards.png",propcfile:"images/board-icons/IconC.png"}[a.board]+'"/>')};
